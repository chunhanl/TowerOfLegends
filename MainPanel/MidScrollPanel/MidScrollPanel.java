...
public class MidScrollPanel extends JPanel{
	...
	public void checkPanelY(){ 		//check if out of bound
		...
		{
			if(thispanelREF.getHeight()<500)thispanelREF.setLocation(0,180);
			else thispanelREF.setLocation(0,550-thispanelREF.getHeight());
	...
	public void setRowsText(int i,String text){
		JLabel Rtext=new JLabel(text,SwingConstants.CENTER);
		Rtext.setLocation(0,0);
		Rtext.setSize(this.panelRows[i].getWidth(),this.panelRows[i].getHeight());
		Rtext.setForeground(Color.ORANGE);
		Rtext.setFont(new Font("Bernard MT Condensed", Font.BOLD, 30));
		panelRows[i].add(Rtext,0);
	}
