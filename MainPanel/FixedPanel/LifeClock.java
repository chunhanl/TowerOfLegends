...
public class LifeClock extends JLabel{
	...
	private Timer time;
	private Timer time2;  //for ticking seconds on clock
	...
	private void drawClock() {
		lifeboard=new JLabel(){
			public void paintComponent(Graphics g){
				...
				g.setFont(new Font("Baskerville Old Face",Font.BOLD,24));
				g.drawString(String.valueOf(new Date().getMinutes())+" : ", 42, 120);
				if(new Date().getSeconds()<10)
					g.drawString("0"+String.valueOf(new Date().getSeconds()), 82, 120);
				else
					g.drawString(String.valueOf(new Date().getSeconds()), 82, 120);
			}
		};
		circle=new JLabel();
